...
class M_about extends CI_Model {
   ...
   function getContent_About($id)
   {
      $sql = "SELECT a.*, c.image submenuImg
               ...
               FROM contents
               WHERE submenu_id != 1 AND status = 1 AND is_delete = 0
   ...
   function getHeaderContent_About($id)
   {
      $sql = "SELECT a.*, c.image
         FROM contents a
         inner join menu c ON c.id=a.submenu_id
         where a.submenu_id = 1 AND a.id_parent = $id AND a.status = 1 AND a.is_delete = 0
